...
@section('content')
    <div class="panel">
        ...
        <div class="mb-5">
            <form class="space-y-5" method="POST" action="{{ route('transport.update', $transport->id) }}">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-12">
                    <div>
                        <label for="type">Nama</label>
                        <input id="type" name="type" type="text" placeholder="nama kapal" value="{{ old('type', $transport->type) }}" class="form-input" autocomplete="">
                    </div>
                </div>
