...
@section('content')
    <div class="panel">
        ...
        <div class="mb-5">
            <form class="space-y-5" method="POST" action="{{ route('penumpang.update', $penumpang->id) }}">
                @method('PUT')
                @csrf
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-12">
                    <div>
                        <label for="type">Type</label>
                        <input id="type" name="type" type="text" placeholder="nama plat" class="form-input" autocomplete="" value="{{ old('type', $penumpang->type) }}">
                    </div>
                    <div>
                        <label for="plat">Nomor</label>
                        <input id="nomor" name="nomor" type="text" placeholder="nama plat" class="form-input" autocomplete="" value="{{ old('nomor' , $penumpang->nomor) }}">
                    </div>
                    <div>
                        <label for="harga">Harga</label>
                        <input id="harga" name="harga" type="text" placeholder="nama harga" class="form-input" autocomplete="" value="{{ old('harga', $penumpang->harga) }}">
                    </div>
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-12 lg:grid-cols-12">
                        <div>
                            ...
                            <select id="ticket_type_id" name="ticket_type_id" class="form-select text-white-dark">
                                @foreach ($tiket as $b)
                                <option value="{{ old('id', $b->id) }}">{{ old('name', $b->name) }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
