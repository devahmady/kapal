...
@section('content')
    <div class="panel">
        ...
        <div class="mb-5">
            <form class="space-y-5" method="POST" action="{{ route('bank.update', $bank->id) }}">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-12">
                    <div>
                        <label for="name">Nama</label>
                        <input id="name" name="name" type="text" placeholder="nama bank" value="{{ old('name', $bank->name) }}" class="form-input" autocomplete="">
                    </div>
                    <div>
                        <label for="rekening">Rekenig</label>
                        <input id="rekening" name="rekening" type="text" placeholder="" value="{{ old('rekening', $bank->rekening) }}" class="form-input" autocomplete="">
                    </div>
                </div>
