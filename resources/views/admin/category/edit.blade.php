...
@section('content')
    <div class="panel">
        ...
        <div class="mb-5">
            <form class="space-y-5" method="POST" action="{{ route('category.update', $category->id) }}">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-12">
                    <div>
                        <label for="name">Nama</label>
                        <input id="name" name="name" type="text" placeholder="nama bank" value="{{ old('name', $category->name) }}" class="form-input" autocomplete="">
                    </div>
