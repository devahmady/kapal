...
@section('content')
    <div class="panel">
        ...
        <div class="mb-5">
            <form class="space-y-5" method="POST" action="{{ route('kendaraan.update', $kendaraan->id) }}">
                @method('PUT')
                @csrf
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-12">
                    <div>
                        <label for="type">Type</label>
                        <input id="type" name="type" type="text" placeholder="nama plat" class="form-input" autocomplete="" value="{{ old('type', $kendaraan->type) }}">
                    </div>
                    <div>
                        <label for="plat">Plat</label>
                        <input id="plat" name="plat" type="text" placeholder="nama plat" class="form-input" autocomplete="" value="{{ old('plat' , $kendaraan->plat) }}">
                    </div>
                    <div>
                        <label for="harga">Harga</label>
                        <input id="harga" name="harga" type="text" placeholder="nama harga" class="form-input" autocomplete="" value="{{ old('harga', $kendaraan->harga) }}">
                    </div>
